...
module.exports = class Expense {
  ...
  static async getExpenseById(id){
    const { rows } = pool.query(`SELECT * FROM monthly_expenses
      WHERE id=$1`, [id]);
  ...
  static async updateExpense(id, { rent, phone, transportation, food, utilities, entertainment, savings }){
    const { rows } = pool.query(`UPDATE monthly_expenses
      SET rent=$2, phone=$3, transportation=$4, food=$5, utilities=$6, entertainment=$7, savings=$7
      WHERE id=$1
